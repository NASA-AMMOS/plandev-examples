...
public record SchedulePriorityActivities(
  ...
  public static class Template {
    public Duration periodStart = Duration.of(12, HOUR);
    public Duration periodDuration = Duration.of(24, HOUR);
    public double altitudeThreshold = 280.0; // km
    public Duration activityGTime = Duration.of(20, HOUR);
    public Duration activityDMinDuration = Duration.of(30, MINUTE);
  }
  ...
  public void run(EditablePlan plan) {
    ...
    // ===== ACTIVITY C: TakeRadarObservation at/below altitude threshold =====
      ...
      if (window.end.longerThan(periodStart) && window.start.shorterThan(periodEnd)) {
        ...
        var activityCDirective = new NewDirective(
            new AnyDirective(Map.of(
                ...
                "duration", new DurationValueMapper().serializeValue(Duration.of(5, MINUTE)),
                "scheduled", SerializedValue.of(true),
                "schedulingPriority", SerializedValue.of(1))),
            "ActivityC_TakeRadarObservation_AltitudeBased",
    ...
    if (activityCTime != null) {
      ...
      if (timeAfterC.longerThan(activityDMinDuration)) {
        plan.create(new NewDirective(
            new AnyDirective(Map.of(
