...
2. Build the action by running `npm run build`. This generates a bundled file at `dist/action.js`, which you can [upload to PlanDev](https://nasa-ammos.github.io/aerie-docs/sequencing/actions/).
3. Fresh requires a valid command dictionary to be [uploaded to PlanDev](https://nasa-ammos.github.io/aerie-docs/command-expansion/upload-command-dictionary/) before it can be invoked.
